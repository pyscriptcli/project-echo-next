...
export async function POST(req: NextRequest) {
  try {
    ...
    const description = [`# IT Asset Request`, `**Request title:** ${title}`, `**Employee:** ${value("employeeName")}`, `**Department:** ${value("department")}`, `**Asset type:** ${value("assetType")}`, `**Needed by:** ${value("neededBy")}`, `**Assignee:** ${value("assignee")}`, `**Team leader approval:** ${form.get("tlApproval") ? "Confirmed" : "Pending"}`, `\n## Reason for request\n${value("reason")}`, `\n## Remarks\n${value("remarks")}`, `\n---\nSubmitted by ${value("requestorName")} (${value("requestorEmail")})`].join("\n");
    ...
    return NextResponse.json({ taskId: task.id, taskUrl: task.url });
