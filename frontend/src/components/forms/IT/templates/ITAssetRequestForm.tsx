...
export default function ITAssetRequestForm({ listId, user }: { listId: string; user?: { name?: string; email?: string } | null }) {
  ...
  return <form onSubmit={submit} className="max-w-[850px] mx-auto border border-slate-300 bg-white shadow-sm p-7 text-[#172033]">
    ...
    <button disabled={saving} className="mt-5 w-full bg-[#003366] text-white py-3 text-sm font-bold flex items-center justify-center gap-2 disabled:opacity-60"><Send size={15} />{saving ? "Submitting…" : "Submit IT Asset Request"}</button>
  </form>;
}
