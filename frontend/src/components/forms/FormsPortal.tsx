...
import { ClipboardList, Settings2, FileEdit, PackageSearch, ShieldCheck, Workflow, Save, Plus, X, AlertTriangle } from "lucide-react";
...
import FormsWorkflowView from "./FormsWorkflowView";
import { PrototypeTourModal } from "./PrototypeTourModal";

type PortalTab = "create" | "track" | "approvals" | "workflow" | "admin";
...
interface FormMapping { id: string; department: string; formType: FormType; approvers: Array<{ stage: number; emails: string[]; requireAll: boolean }>; }
...
function AdminConfiguration({ userEmail }: { userEmail: string }) {
  ...
  const save = async (next: FormsConfig = config) => {
    ...
    setNotice(res.ok ? "Configuration saved." : "Saved locally. Supabase persistence is not configured yet.");
  ...
  const ensureMapping = () => mapping || { id: `${selectedDepartment}-${selectedForm}`, department: selectedDepartment, formType: selectedForm, approvers: STAGES.map((_, stage) => ({ stage, emails: [], requireAll: false })) };
  ...
  return <div className="space-y-5">
    ...
    <section className="panel"><h2 className="text-lg mb-3">Departments</h2><div className="flex flex-wrap gap-2 mb-3">{config.departments.map((department) => <span key={department} className="border border-gray-200 px-3 py-1.5 text-xs">{department}</span>)}</div><div className="flex gap-2 max-w-md"><input className="border border-gray-300 px-3 py-2 text-xs flex-1" placeholder="Add department" value={newDepartment} onChange={(e) => setNewDepartment(e.target.value)} /><button className="btn-outline flex items-center gap-1" onClick={addDepartment}><Plus size={14} /> Add</button></div></section>
    <section className="panel"><h2 className="text-lg mb-1">Approval routing</h2><p className="text-xs text-gray-500 mb-4">Map approvers independently for every department, form, and fixed workflow stage.</p><div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-5"><label className="text-xs font-bold text-gray-600">Department<select className="mt-1 w-full border border-gray-300 px-3 py-2 bg-white font-normal" value={selectedDepartment} onChange={(e) => setSelectedDepartment(e.target.value)}>{config.departments.map((department) => <option key={department}>{department}</option>)}</select></label><label className="text-xs font-bold text-gray-600">Form<select className="mt-1 w-full border border-gray-300 px-3 py-2 bg-white font-normal" value={selectedForm} onChange={(e) => setSelectedForm(e.target.value as FormType)}>{Object.entries(FORM_LABELS).map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select></label></div><div className="space-y-2">{STAGES.map((stageName, index) => { const stage = current.approvers[index] || { stage: index, emails: [], requireAll: false }; return <div key={stageName} className="grid grid-cols-1 md:grid-cols-[1.3fr_1fr_auto] gap-2 items-center border border-gray-200 p-3"><div><div className="text-xs font-bold text-[#003366]">{index + 1}. {stageName}</div><div className="text-[10px] text-gray-400">{stage.requireAll ? "All assigned approvers required" : "Any one approver can advance"}</div></div><input className="border border-gray-300 px-3 py-2 text-xs" placeholder="approver@primephilippines.com (comma separated)" value={stage.emails.join(", ")} onChange={(e) => { const emails = e.target.value.split(",").map(normalizeEmail).filter(Boolean); const next = { ...current, approvers: current.approvers.map((item) => item.stage === index ? { ...item, emails } : item) }; updateMapping(next); }} /><label className="text-[11px] flex items-center gap-1.5 whitespace-nowrap"><input type="checkbox" checked={stage.requireAll} onChange={(e) => { const next = { ...current, approvers: current.approvers.map((item) => item.stage === index ? { ...item, requireAll: e.target.checked } : item) }; updateMapping(next); }} /> All required</label></div>; })}</div></section>
  </div>;
}

export default function FormsPortal({ user, initialTab = "create" }: { user?: { username?: string; email?: string } | null; initialTab?: PortalTab }) {
  const [tab, setTab] = useState<PortalTab>(initialTab);
  const searchParams = useSearchParams();
  useEffect(() => { const requested = searchParams.get("tab") as PortalTab | null; if (requested && ["create", "track", "approvals", "workflow", "admin"].includes(requested)) setTab(requested); }, [searchParams]);
  ...
  const tabs: Array<{ id: PortalTab; label: string; icon: typeof FileEdit }> = [{ id: "create", label: "New Form", icon: FileEdit }, { id: "track", label: "Track Status", icon: PackageSearch }, { id: "approvals", label: "Approvals", icon: ShieldCheck }, { id: "workflow", label: "Workflow", icon: Workflow }];
  return <div className="space-y-4"><div className="flex flex-wrap items-center justify-between gap-3 border-b border-gray-200 pb-3"><div><h1 className="text-2xl flex items-center gap-2"><ClipboardList size={22} className="text-[#C9AB4C]" /> Forms Portal</h1><p className="text-xs text-gray-500 uppercase tracking-wider mt-1">ClickUp requests, approvals, and progress</p></div><div className="flex flex-wrap gap-1">{tabs.map(({ id, label, icon: Icon }) => <button key={id} onClick={() => setTab(id)} className={`px-3 py-2 text-xs font-bold flex items-center gap-1.5 border ${tab === id ? "bg-[#003366] text-white border-[#003366]" : "bg-white text-[#003366] border-gray-200 hover:border-[#C9AB4C]"}`}><Icon size={14} />{label}</button>)}{isAdmin && <button onClick={() => setTab("admin")} className={`px-3 py-2 text-xs font-bold flex items-center gap-1.5 border ${tab === "admin" ? "bg-[#1b1d1e] text-[#C9AB4C] border-[#1b1d1e]" : "bg-white text-[#003366] border-gray-200 hover:border-[#C9AB4C]"}`}><Settings2 size={14} />Admin</button>}</div></div>{tab === "create" && <FormsCreateView user={{ name: user?.username, email: user?.email }} />}{tab === "track" && <FormsTrackView />}{tab === "approvals" && <FormsApprovalsView />}{tab === "workflow" && <FormsWorkflowView />}{tab === "admin" && <AdminConfiguration userEmail={user?.email || ""} />}<PrototypeTourModal /></div>;
}
