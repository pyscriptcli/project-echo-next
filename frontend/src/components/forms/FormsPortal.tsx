...
function AdminConfiguration({ userEmail }: { userEmail: string }) {
  ...
  const ensureMapping = () => mapping || { id: `${selectedDepartment}-${selectedForm}`, department: selectedDepartment, formType: selectedForm, formLabel: selectedForm === "it-asset-request-form" ? "IT Asset Request Form" : FORM_LABELS[selectedForm as FormType], listId: "", approvers: STAGES.map((_, stage) => ({ stage, emails: [], requireAll: false })) };
