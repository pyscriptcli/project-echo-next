...
export default function FormsCreateView({ user }: { user?: FormsUser | null }) {
  ...
  if (!selectedForm) return <div className="bg-bg-primary text-[#0C0C0E] py-6 px-3 sm:px-6"><div className="max-w-6xl mx-auto"><h2 className="text-2xl font-semibold text-[#003366] mb-1">New Form</h2><p className="text-sm text-gray-500 mb-6">Choose a department, then choose the form you want to submit.</p><div className="grid grid-cols-1 lg:grid-cols-[280px_1fr] border border-gray-200 bg-white min-h-[360px]"><aside className="border-r border-gray-200 p-4"><div className="text-[10px] font-bold uppercase tracking-widest text-gray-400 mb-3">Department</div><div className="space-y-2">{departments.map((name) => <button key={name} onClick={() => setCategory(name)} className={`w-full text-left px-4 py-3 border transition ${category === name ? "border-[#003366] bg-[#003366] text-white" : "border-gray-200 hover:border-[#C9AB4C] hover:bg-[#fffdf6]"}`}><div className="font-bold text-sm">{name}</div></button>)}</div></aside><section className="p-6"><div className="text-[10px] font-bold uppercase tracking-widest text-gray-400 mb-1">Available forms</div><div className="text-sm text-gray-500 mb-4">{category ? `${category} forms` : "Select a department on the left"}</div><div className="grid grid-cols-1 md:grid-cols-2 gap-3">{["Request for Payment", "Purchase Order", "Petty Cash Voucher"].map((form) => <button key={form} disabled={!category} onClick={() => setSelectedForm(form)} className="min-h-28 text-left p-5 border border-gray-200 enabled:hover:border-[#C9AB4C] enabled:hover:shadow-sm disabled:opacity-40 transition"><div className="font-bold text-[#003366]">{form}</div><div className="text-xs text-gray-400 mt-2">{category ? `Create for ${category}` : "Choose a department first"}</div></button>)}</div></section></div></div></div>;
  return (
    <Suspense
      fallback={
