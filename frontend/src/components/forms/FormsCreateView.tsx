    ...
    <div className="bg-bg-primary text-[#0C0C0E] py-0 px-3 sm:px-6">
      <div className="max-w-[1500px] mx-auto">
        <div className="grid grid-cols-1 xl:grid-cols-[300px_minmax(0,1fr)] gap-6">
          <aside className="border border-gray-200 bg-[#FFFCFB] p-5 self-start">
            <div className="text-[10px] font-bold uppercase tracking-widest text-gray-400 mb-3">Departments</div>
            <div className="space-y-2">
              {departments.map((name) => <button key={name} onClick={() => setCategory(name)} className={`w-full text-left px-4 py-4 border transition ${category === name ? "border-[#C9AB4C] bg-[#FFFCFB] text-[#003366]" : "border-gray-200 hover:border-[#C9AB4C]"}`}><div className="font-bold text-sm">{name}</div></button>)}
            </div>
          </aside>
          <section className="border border-gray-200 bg-[#FFFCFB] p-5 min-w-0">
            {category === "IT" ? (
              <div className="relative mb-5 flex flex-col gap-3 border border-slate-300 bg-[#FFFCFB] px-4 py-3 shadow-sm lg:flex-row lg:items-center">
            ...
            {listId ? <Suspense fallback={<div className="min-h-64 flex items-center justify-center text-sm text-gray-500">Loading form…</div>}>{category === "IT" ? itFormKey === "it-helpdesk-support-form" ? <ITHelpdeskSupportForm listId={listId} user={user} /> : itFormKey === "it-bug-error-report-form" ? <ITBugErrorReportForm listId={listId} user={user} /> : <ITAssetRequestForm listId={listId} user={user} /> : <RfpAppContent user={user} listId={listId} />}</Suspense> : <div className="border border-slate-200 bg-[#FFFCFB] px-4 py-3 text-sm text-slate-600">No forms available.</div>}
          </section>
        </div>
      </div>
      {submissionResult && <div className="fixed inset-0 z-[100] flex items-center justify-center bg-[#001d3d]/55 px-4" role="dialog" aria-modal="true" aria-labelledby="submission-confirmation-title"><div className="w-full max-w-md border border-[#C9AB4C] bg-[#FFFCFB] p-7 shadow-2xl"><CheckCircle2 className="mx-auto text-emerald-600" size={42} /><h3 id="submission-confirmation-title" className="mt-3 text-center font-serif text-2xl font-bold italic text-[#003366]">Successfully submitted</h3><p className="mt-2 text-center text-sm text-slate-600">Your request has been created in ClickUp.</p>{submissionResult.formRequestId && <p className="mt-4 border border-[#C9AB4C] bg-[#FFFCFB] px-3 py-2 text-center text-xs font-bold text-[#003366]">{submissionResult.formRequestId}</p>}{submissionResult.taskUrl && <a className="mt-4 block text-center text-sm font-bold text-[#003366] underline" href={submissionResult.taskUrl} target="_blank" rel="noreferrer">Open in ClickUp</a>}<button onClick={() => setSubmissionResult(null)} className="mt-6 w-full bg-[#003366] py-3 text-sm font-bold text-white">Done</button></div></div>}
    </div>
  );
}
