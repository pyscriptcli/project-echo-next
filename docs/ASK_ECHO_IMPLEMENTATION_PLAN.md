...
Ask Echo becomes a trustworthy, read-only ClickUp assistant. It answers questions across the Project Echo areas backed by ClickUp, supports follow-up questions during the current session, and shows the source behind every factual answer.
...
- Ask Echo searches ClickUp records associated with Meetings, Tasks, Notebook, Forms, Demands, and Market Insights.
...
- Each source inherits its matching page permission. A restricted page contributes no data to retrieval or prompts.
...
3. The server determines the user's allowed pages and enables only the matching ClickUp sources.
4. A retrieval module selects a small set of relevant records from those enabled sources.
...
- Normalize allowed ClickUp tasks, daily logs, forms, demands, market insights, meetings, and meeting items into searchable records with stable IDs.
...
- The model receives only relevant records from pages the user can open.
...
- Workspace-wide ClickUp sources that do not yet map to a Project Echo page and permission.
